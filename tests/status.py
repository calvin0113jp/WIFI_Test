...
# History
# 2019/1/7 Modify Ucishow
# 2019/1/11 Mofify Logread:try:except / sendcontrol
...
class Logread(rootfs_boot.RootFSBootTest):
    ...
    def runTest(self):
        ...
        try:
            board.expect('WRC',timeout=10)            
            board.expect(prompt,timeout=60)
            board.sendcontrol('c')
        except:
            pass
...
class UciShow(rootfs_boot.RootFSBootTest):
	...
	board.sendline('\nls -l /etc/config/')
        ...
        board.expect('(\d+)\r\n')
	num_files = int(board.match.group(1))
        board.sendline('uci show | wc -l')
	board.expect('(\d+)\r\n')
	num_files1 = int(board.match.group(1))
        self.result_message = 'Dumped all current /etc/config from %s files and uci settings from %s files.' % (num_files, num_files1)
