...
# History 
# 2019/1/23 1. Modify BitTorrentSingle / BitTorrentB2B
#              board.send(cat /proc/net/nf_conntrack) to cat /proc/sys/net/netfilter/nf_conntrack_count
#           2. BitTorrentClient - Add apt-get install bittornado and modify the download link
#           3. BitTorrentBasic  - Add nf_conn_track_count
...
class BitTorrentSingle(BitTorrentBasic):
    ...
    def runTest(self):
        ...
        board.sendline('cat /proc/sys/net/netfilter/nf_conntrack_count | grep dst=%s.*dport=%s' % (ip, port))
        board.expect(prompt)
...
class BitTorrentClient(rootfs_boot.RootFSBootTest):
    def runTest(self):
        ...
        lan.sendline('apt-get install -y bittornado')
        lan.expect(prompt, timeout=60)

        for i in range(10):
            lan.sendline("btdownloadheadless 'https://torrent.fedoraproject.org/torrents/Fedora-KDE-Live-i386-28.torrent'")
            lan.expect('saving:')
            done = False
            while not done:
